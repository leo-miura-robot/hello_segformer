...
class ROS_Segformer:
    def __init__(self):
        ...
        # self.checkpoint = '/home/envy/catkin_ws/src/hello_segformer/script/checkpoints/fch_yugo_4_scapes_b0_iter_4000.pth'
        self.checkpoint = '/home/envy/catkin_ws/src/hello_segformer/script/checkpoints/fch_yugo_4_scapes_b0_iter_28000.pth'
    ...
    def publication(self):
        plot_resize_h = int(480/3)
        plot_resize_w = int(640/3)

        rospy.loginfo('---')

        image = self.cv_image
        image = cv2.resize(image, (256, 256)) # <--- Add resizing
        gimage = cv2.resize(self.cv_image, (plot_resize_w, plot_resize_h)) # <--- Add resizing
        cv2.imshow('GAN-generated image', gimage)
        ...
        segimage = cv2.resize(self.cv_segmentation_map, (plot_resize_w, plot_resize_h)) # <--- Add resizing
        cv2.imshow('image segmentation', segimage)
        cv2.waitKey(3)
